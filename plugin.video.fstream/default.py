...
class main:
    ...
    def parseUrl(self):
        # Exclue les appels par des plugins qu'on ne sait pas gérer, par exemple : plugin://plugin.video.fstream/extrafanart
        ...
        #     function()
        #     return
        #Nouveau chargement de Home
        if not oInputParameterHandler.exist('site'):

            token = auth.get_token()
            home = __import__('resources.lib.home', fromlist=['home']).cHome()

            if not token or token == "":
                home.loginScreen()
                return

            function = getattr(home, 'load')
            function()
            return
...
# RunAddon(plugin.video.fstream,function=setSettings&id1=plugin_cinemay_com&value1=true
