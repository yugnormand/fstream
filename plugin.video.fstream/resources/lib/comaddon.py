...
the python script "\plugin.video.fstream\default.py" has left several classes in memory that we couldn't clean up. The classes include: class XBMCAddon::xbmcaddon::Addon

# https://stackoverflow.com/questions/26588266/xbmc-addon-memory-leak
"""
ADDONVS = xbmcaddon.Addon('plugin.video.fstream')  # singleton
...
class dialog:
    ...
    def VSinfo(self, desc, title='fstream', iseconds=1, sound=False):
...
class listitem(xbmcgui.ListItem):
    ...
    def addMenu(self, sFile, sFunction, sTitle, oOutputParameterHandler=False):
        sPluginPath = 'plugin://plugin.video.fstream/'  # cPluginHandler().getPluginPath()
...
class siteManager:
    ...
    def __init__(self):
        ...
        self.defaultPath = VSPath('special://home/addons/plugin.video.fstream/resources/sites.json')
        self.defaultData = None

        # Propriétés selon le profil        
        name = VSProfil()
        if name == 'Master user':   # Le cas par defaut
            path = VSPath('special://home/userdata/addon_data/plugin.video.fstream/sites.json')
        else:
            path = VSPath('special://home/userdata/profiles/' + name + '/addon_data/plugin.video.fstream/sites.json')
